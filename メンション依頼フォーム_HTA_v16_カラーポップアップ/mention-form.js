...
function setRequesterName(){
    var displayName="",userName="",domainName="",el=$("requesterName");
    try{
        var network=new ActiveXObject("WScript.Network");
        userName=String(network.UserName||"");
        domainName=String(network.UserDomain||"");
        try{ displayName=String(GetWindowsDisplayNameHTA()||""); }catch(nameErr){ displayName=""; }
        if(!displayName){ displayName=userName; }
        if(el){ el.innerText=displayName; el.title=domainName+"\\"+userName; }
    }catch(err){ if(el){ el.innerText=userName||"取得できませんでした"; } }
}
...
function hideAppForLayout(){ var app=$("app"); if(app){ app.style.visibility="hidden"; } }
...
function resizeApp(centerOnFirst){
    hideAppForLayout();
    if(layoutTimer){ window.clearTimeout(layoutTimer); layoutTimer=null; }
    layoutTimer=window.setTimeout(function(){
        try{
            var app=$("app");
            if(!app){ return; }
            document.body.style.zoom="1";
            var chromeW=70,chromeH=82;
            var naturalW=app.offsetWidth,naturalH=app.offsetHeight;
            var maxW=screen.availWidth-18,maxH=screen.availHeight-18;
...
function hasAnyInput(req){
    return !!(req.organization||req.caName||req.proxyOrganization||req.proxyCA1||req.proxyCA2||req.proxyCA3||req.mailMemo||req.processedAt||req.dueDate||req.shortTime||req.urgent||req.type);
}
...
function validateForm(){
    ...
    for(i=1;i<=visibleRequestCount;i++){
        ...
        if(!normalAny&&!proxyAny){
            markError("org"+i); markError("ca"+i); markError("proxyOrg"+i); markError("proxyCA"+i+"_1");
...
function showValidationModal(requestNo,message,focusId){
    var overlay=$("modalOverlay"),modal=$("validationModal"),title=$("modalTitle"),msg=$("modalMessage");
    ...
    try{ $("modalOkButton").focus(); }catch(err){}
}

function closeValidationModal(){
    var overlay=$("modalOverlay");
    if(overlay){ overlay.className="modal-overlay hidden"; }
    if(modalFocusTarget){ focusField(modalFocusTarget); }
    modalFocusTarget="";
}

function clearErrors(){
    var ids=["requestBase","org1","ca1","proxyOrg1","proxyCA1_1","mailMemo1","processedAt1","type1","org2","ca2","proxyOrg2","proxyCA2_1","mailMemo2","processedAt2","type2","org3","ca3","proxyOrg3","proxyCA3_1","mailMemo3","processedAt3","type3"];
    var i,el;
    for(i=0;i<ids.length;i++){
        el=$(ids[i]);
        if(el){ el.className=String(el.className||"").replace(/\berror-field\b/g,"").replace(/^\s+|\s+$/g,""); }
    }
}
...
function markError(id){
    ...
    if(cls.indexOf("error-field")<0){ el.className=(cls+" error-field").replace(/^\s+|\s+$/g,""); }
}

function focusField(id){ try{ var el=$(id); if(el){ el.focus(); } }catch(err){} }
function trimValue(id){ return val(id).replace(/^\s+|\s+$/g,""); }
...
function sendRequest(){
    ...
    var lines=[];
    for(i=0;i<requests.length;i++){ lines.push(makeCsvLine(requestId,sentAt,val("requestBase"),requester,requests[i])); }
    setStatus("送信中...");
    $("sendButton").disabled=true;
    writeCsvWithLock(
        val("requestBase"),lines,0,
        function(){
            setStatus("送信しました");
...
function resetAfterSend(){
    var i;
    for(i=1;i<=3;i++){
        setValue("org"+i,""); setValue("ca"+i,""); setValue("proxyOrg"+i,"");
        setValue("proxyCA"+i+"_1",""); setValue("proxyCA"+i+"_2",""); setValue("proxyCA"+i+"_3","");
        setValue("mailMemo"+i,""); setValue("processedAt"+i,""); setValue("dueDate"+i,"");
        setChecked("short"+i,false); setChecked("urgent"+i,false);
        if($("type"+i)){ $("type"+i).selectedIndex=0; }
    }
    if($("request2")&&$("request2").className.indexOf("hidden")<0){ $("request2").className+=" hidden"; }
    if($("request3")&&$("request3").className.indexOf("hidden")<0){ $("request3").className+=" hidden"; }
...
function writeCsvWithLock(baseName,lines,retryCount,onSuccess,onFailure){
    ...
    try{
        ...
        if(retryCount<RETRY_MAX){
            window.setTimeout(function(){ writeCsvWithLock(baseName,lines,retryCount+1,onSuccess,onFailure); },RETRY_INTERVAL_MS);
        }else{
...
function makeHeaderLine(){
    return csvJoin(["送信日時","RequestID","拠点","依頼者","依頼番号","組織","CA名","代理CA組織","代理CA1","代理CA2","代理CA3","メールメモ","処理日時","期日","時短","至急","タイプ"]);
}

function makeCsvLine(requestId,sentAt,baseName,requester,req){
    return csvJoin([
        sentAt,requestId,baseName,requester,String(req.requestNo),
        req.organization,req.caName,req.proxyOrganization,req.proxyCA1,req.proxyCA2,req.proxyCA3,
...
function csvJoin(values){ var out=[],i; for(i=0;i<values.length;i++){ out.push(csvEscape(values[i])); } return out.join(","); }
function csvEscape(value){ var s=String(value==null?"":value); s=s.replace(/"/g,'""'); return '"'+s+'"'; }

function createRequestId(requester){
    var now=new Date();
    var userPart=String(requester||"USER").replace(/[\\\/:*?"<>|\s]/g,"").substring(0,12);
    var rand=Math.floor(Math.random()*65536).toString(16).toUpperCase();
    while(rand.length<4){ rand="0"+rand; }
    return formatDate(now)+"-"+pad2(now.getHours())+pad2(now.getMinutes())+pad2(now.getSeconds())+"-"+userPart+"-"+rand;
}
...
function sanitizeFileName(name){ return String(name||"未選択").replace(/[\\\/:*?"<>|]/g,"_"); }
function setStatus(message){ var el=$("statusText"); if(el){ el.innerText=message||""; } }
