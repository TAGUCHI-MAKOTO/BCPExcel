...
/* =========================================================
   DEMO MODE v26
   初回表示から「エラー文＋再送ボタン」を完成状態で表示
   CSV / Pending / PowerShell / Windows通知は一切実行しない
   ========================================================= */

var DEMO_MODE = true;

function demoShowFailure(){
    var area=$("pendingRetryArea");
    var text=$("pendingRetryText");
    var retry=$("retryPendingButton");
    var send=$("sendButton");
    ...
    if(text){
        text.className="pending-retry-text failed";
        text.innerText="送信できませんでした│未送信 1件";
    }
...
/*
  デモ専用init。
  ファイルシステム・Pending・PowerShellへ一切触れない。
  アプリが画面へ表示される前に失敗UIを完成させる。
*/
function initApp(){
    populateTypes();
...
function launchBackgroundWorker(pendingPath,csvFolder){
    return false;
}
...
function sendRequest(){
    try{
        showColorMessage(
            "デモ版",
            "このデモではCSV書き込み・Pending作成・PowerShell起動は行いません。"
        );
    }catch(e){}
}
